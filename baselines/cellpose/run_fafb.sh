...
MODEL_PATH="/nvme2/mingzhi/NucCorr/TMI/OtherMethods/Cellpose/checkpoint_fafb/models/NucCorr_Cellpose_v3"

# 结果保存根目录
RESULT_DIR="/nvme2/mingzhi/NucCorr/TMI/OtherMethods/Cellpose/results/fafb"
mkdir -p $RESULT_DIR

# 推理核心参数配置
ANISOTROPY=1.25    # Z轴与XY轴的比例
DIAMETER=20.0      # 细胞核平均直径
CELL_PROB=0.5      # 细胞概率阈值（越大越严格）
FLOW_THRES=0.4     # 梯度流阈值（越大越严格，减少合并）
DEVICE="cuda:3"    # 指定显卡
